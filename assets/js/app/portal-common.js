...
export async function getPortalContext() {
  ...
  if (ctx.role === 'player') {
    const { data, error } = await supabase
      .from('players')
      .select('id, team_id, full_name, jersey_number, primary_position, secondary_position, status, profile_id, teams(name)')
...
export async function fetchTeamBundle(teamId) {
  ...
  const [teamRes, playersRes, coachesRes, tacticsRes, sessionsRes, matchesRes] = await Promise.all([
    ...
    supabase.from('players').select('id,profile_id,full_name,jersey_number,primary_position,status,image_url').eq('team_id', teamId).order('jersey_number', { ascending: true, nullsFirst: false }).order('full_name'),
